class Job < ApplicationRecord
  ...
  validates :title, :description, :salary_range, :level, :type_hiring, :requirements,
            :deadline_application, :total_vacancies, presence: true

  def applied(candidate)
    return false if candidates.exists?(candidate.id)
